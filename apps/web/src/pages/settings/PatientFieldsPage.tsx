...
import { PATIENT_FIELD_TYPES, type PatientFieldDefinition } from '@clinic-care/shared-types';
...
const FIELD_TYPE_LABEL: Record<(typeof PATIENT_FIELD_TYPES)[number], string> = {
  TEXT: 'Text',
  NUMBER: 'Number',
  DATE: 'Date',
  SELECT: 'Dropdown',
...
const patientFieldSchema = z.object({
  ...
  fieldType: z.enum(PATIENT_FIELD_TYPES),
  optionsText: z.string().optional(),
  required: z.boolean().optional(),
});

type PatientFieldFormValues = z.infer<typeof patientFieldSchema>;
...
export function PatientFieldsPage() {
  ...
  const { data: fields, isLoading } = usePatientFieldsQuery();
  const { create, update, deactivate, reactivate } = usePatientFieldMutations();
  ...
  const openEdit = (field: PatientFieldDefinition) => {
    form.reset({
      key: field.key,
      label: field.label,
  ...
  const onSubmit = async (values: PatientFieldFormValues) => {
    ...
    try {
      if (editTarget) {
        // Built-in fields are tied to a real Patient column — its type can't change
  ...
  const onToggleActive = async (field: PatientFieldDefinition) => {
    try {
      if (field.isActive) {
  ...
  const columns: ColumnDef<PatientFieldDefinition>[] = [
    {
      accessorKey: 'label',
      header: 'Label',
      cell: ({ row }) => (
    ...
    { accessorKey: 'fieldType', header: 'Type', cell: ({ getValue }) => FIELD_TYPE_LABEL[getValue<PatientFieldDefinition['fieldType']>()] },
    {
      ...
      ? [
          {
            id: 'actions',
            header: 'Actions',
            cell: ({ row }: { row: { original: PatientFieldDefinition } }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[var(--color-navy)]">Patient Fields</h1>
          <p className="text-sm text-gray-500">
            Choose what extra details to collect from patients, and which of them are mandatory.
          </p>
        </div>
        {canEdit && (
          <button
      ...
      <FormModal
        ...
        title={editTarget ? `Edit ${editTarget.label}` : 'Add Patient Field'}
        size="sm"
        onClose={() => {
          setIsAddOpen(false);
          setEditTarget(null);
      ...
      >
        <form className="flex flex-col gap-4">
          ...
          <div>
            ...
            {editTarget?.isCore && (
              <p className="mt-1 text-xs text-gray-400">Built-in field — type matches the real patient record column.</p>
            )}
          </div>
          ...
          {fieldType === 'SELECT' && !editTarget?.isCore && (
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[var(--color-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]"
                placeholder="Comma separated, e.g. HDFC ERGO, Star Health, Other"
              />
          ...
          <label className={cn('flex items-center gap-2 text-sm text-gray-700')}>
            ...
            />
            Mandatory — patients must provide this
          </label>
        </form>
      </FormModal>
      ...
      <ConfirmDialog
        ...
        description={
          confirmTarget?.isActive
            ? "It won't show on the patient form and its required check stops applying. Existing saved values are kept, and this can be reversed any time."
            : 'It will show on the patient form again immediately.'
        }
